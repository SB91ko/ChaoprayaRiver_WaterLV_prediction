...
#-----------------------Baseline / Hybrid -----------------------------------#
save_path =host_path+'06_wtXY_lv2'
...
def wav_dl_wav_run(model_list,batch,syn):
    ...
    trainPredict = inverse_WT([cA3ytrain,cD3ytrain])
    testPredict = inverse_WT([cA3ytest,cD3ytest])

    _,Y=autosplit(data_mar)
    y_train,y_test = Y[0],Y[1]

    scaler_t = StandardScaler()
    scaler_t.fit(y_train)
...
for batch_size in [16,32]:
    model = [build_cnn1d(),build_cnn1d()]
    wav_dl_wav_run(model,batch_size,'wCNNx2_MAR{}_b{}_Tin{}_{}'.format(cutoff,batch_size,n_past,syn))
    model = [build_cnn1d(),build_lstm()]
    wav_dl_wav_run(model,batch_size,'wlstmx2_MAR{}_b{}_Tin{}_{}'.format(cutoff,batch_size,n_past,syn))
    model = [build_lstm_v2(),build_lstm_v2()]
    wav_dl_wav_run(model,batch_size,'wlstmV2x2_MAR{}_b{}_Tin{}_{}'.format(cutoff,batch_size,n_past,syn))
    model = [build_lstm(),build_cnn1d()]
    wav_dl_wav_run(model,batch_size,'wlstmCNN_MAR{}_b{}_Tin{}_{}'.format(cutoff,batch_size,n_past,syn))
    model = [build_ann(),build_ann()]
    wav_dl_wav_run(model,batch_size,'wANNx2_MAR{}_b{}_Tin{}_{}'.format(cutoff,batch_size,n_past,syn))