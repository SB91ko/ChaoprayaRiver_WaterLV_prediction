from Hour_Multi_DL import build_cnn1d
...
def run_code(model,batch_size,syn,zoom=False):
    ...
    #################################################
    validataion = ([cAX_test_sc, cDX_test_sc],y_test)
    history = model.fit(x=[cAX_train_sc, cDX_train_sc], y=y_train,epochs=epochs,batch_size=batch_size,verbose=verbose,validation_data = validataion,callbacks=callbacks)
    ...
    plt.clf()
    #################################################
    trainPredict = model.predict([cAX_train_sc, cDX_train_sc])
    testPredict = model.predict([cAX_test_sc, cDX_test_sc])
    
    a=pd.DataFrame(trainPredict)
    b=pd.DataFrame(testPredict)
    pd.concat([a,b]).to_csv('test_file.csv')

    scale_y_test,scale_testPredict = record_list_result(syn,df_scaled,mode,y_train,y_test,trainPredict,testPredict,target,batch_size,save_path,n_past,n_features,n_future,scaler_t=scaler_tar)
