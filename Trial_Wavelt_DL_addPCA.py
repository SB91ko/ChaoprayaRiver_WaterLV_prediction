...
def build_cnn_auto():
    ...
    en = layers.LSTM(200, activation="relu",name="cD_LSTM")(inputD)
    en = layers.BatchNormalization()(en)
    de = layers.RepeatVector(n_future)(en)
    de = layers.LSTM(200, activation="relu", return_sequences=True)(de)
    de = layers.BatchNormalization()(de)
    de = layers.TimeDistributed(layers.Dense(100, activation='relu'))(de)
    de = layers.TimeDistributed(layers.Dense(100))(de)
    de = layers.LSTM(100, activation="sigmoid")(de)
    
    #combining 2 lstm
    com = layers.concatenate([x, de])
